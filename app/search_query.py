...
def index_document(es, index_name, chunk, idx):
    """
    ...
    response = es.index(index=index_name, id=idx, document=document)
    return response
...
def process_pdf_to_es(es, index_name, pdf_path):
    ...
    for idx, chunk in enumerate(chunked_documents):
        response = index_document(es, index_name, chunk, idx)
        print(f"Indexed chunk {idx}: {response}")

@click.command()
@click.option('--index_name', help='The name of the Elasticsearch index.')
@click.option('--pdf_path', help='The path to the PDF file to be processed.')
